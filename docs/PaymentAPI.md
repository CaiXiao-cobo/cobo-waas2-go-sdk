...
> ListBulkSends200Response ListBulkSends(ctx).Limit(limit).Before(before).After(after).RequestId(requestId).Execute()
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.ListBulkSends(ctx).Limit(limit).Before(before).After(after).RequestId(requestId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PaymentAPI.ListBulkSends``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListBulkSends`: ListBulkSends200Response
	fmt.Fprintf(os.Stdout, "Response from `PaymentAPI.ListBulkSends`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListBulkSendsRequest struct via the builder pattern
